...
import { listRecoveryTrend, listTreatmentCategoryStats, type RecoveryTrendPoint, type TreatmentCategoryStat } from '../utils/doctorDashboardApi';
...
export function TreatmentStatusTab() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    Promise.all([listTreatmentCategoryStats(ac.signal), listRecoveryTrend(14, ac.signal)])
      .then(([cats, trend]) => {
        ...
        >
          ...
          <button
            onClick={() => {
              setLoading(true);
              setError(null);
              Promise.all([listTreatmentCategoryStats(), listRecoveryTrend(14)])
                .then(([cats, trend]) => {
                  setTreatmentData(cats);
                  setRecoveryTrend(trend);
                })
