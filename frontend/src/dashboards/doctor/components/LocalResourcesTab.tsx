...
export function LocalResourcesTab() {
  ...
  const reloadResources = (signal?: AbortSignal) =>
    Promise.all([listResourceMedicines(signal), listResourceEquipment(signal), listResourceStaff(signal)]).then(([m, e, s]) => {
      setMedicines(m);
      setEquipment(e);
      setStaff(s);
    });
