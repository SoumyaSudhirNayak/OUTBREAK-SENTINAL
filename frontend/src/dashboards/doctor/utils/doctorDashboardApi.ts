...
export async function listLiveTreatmentCategoryStats(signal?: AbortSignal): Promise<TreatmentCategoryStat[]> {
  const resp = await fetch(`${getApiBaseUrl()}/treatment/live-category-stats`, { signal })
  if (!resp.ok) throw new Error(`Failed to load live treatment stats: ${resp.status}`)
  return (await resp.json()) as TreatmentCategoryStat[]
}
...
export async function listLiveRecoveryTrend(days = 14, signal?: AbortSignal): Promise<RecoveryTrendPoint[]> {
  const resp = await fetch(`${getApiBaseUrl()}/treatment/live-recovery-trend?days=${encodeURIComponent(String(days))}`, { signal })
  if (!resp.ok) throw new Error(`Failed to load live recovery trend: ${resp.status}`)
  return (await resp.json()) as RecoveryTrendPoint[]
}
