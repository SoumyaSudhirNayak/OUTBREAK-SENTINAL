...
import { createStockUsage, listVehicleStock, resolveVehicleId, type VehicleStockUsage } from '../utils/medicalVehicleApi';
...
export default function Stock() {
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        if (msg?.type === 'treatment.stock_updated' && msg?.vehicle_id === id && msg?.stock_usage) {
          setEntries((prev) => [msg.stock_usage, ...prev]);
        }
      } catch {
        return;
      }
  ...
  const handleSubmit = () => {
    ...
    createStockUsage({
      vehicle_id: id,
      outbreak_id: (window.localStorage.getItem('mobilevan.activeOutbreakId') ?? '').trim() || null,
